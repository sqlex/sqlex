...
public class InvocationProxy implements InvocationHandler {
    ...
    public InvocationProxy(TransactionManager transactionManager, ParameterConverterRegistry parameterConverterRegistry, ExceptionTranslator translator) {
                ...
                if (methodProxy == null) {
                    ...
                    if (method.getDeclaredAnnotation(SqlExSelect.class) != null) {
                        if (method.getDeclaredAnnotation(SqlExPaged.class) != null) {
                            methodProxy = new SelectPagedMethodProxy(method, transactionManager, parameterConverterRegistry, translator);
                        } else if (method.getDeclaredAnnotation(SqlExOneRow.class) != null) {
                            methodProxy = new SelectOneRowMethodProxy(method, transactionManager, parameterConverterRegistry, translator);
                        } else {
                            methodProxy = new SelectMethodProxy(method, transactionManager, parameterConverterRegistry, translator);
                        }
                    } else if (method.getAnnotation(SqlExInsert.class) != null) {
                        methodProxy = new InsertMethodProxy(method, transactionManager, parameterConverterRegistry, translator);
                    } else if (method.getAnnotation(SqlExUpdate.class) != null
                            || method.getAnnotation(SqlExDelete.class) != null) {
                        methodProxy = new UpdateDeleteMethodProxy(method, transactionManager, parameterConverterRegistry, translator);
                    } else {
