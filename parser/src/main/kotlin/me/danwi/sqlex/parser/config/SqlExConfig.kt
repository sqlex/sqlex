...
fun parseSqlExConfig(content: String): SqlExConfig {
    try {
        ...
        throw SqlExParserException("配置文件错误, 缺少 ${e.path.toHumanReadableString()} 配置, (位置 ${e.line}:${e.column})")
    } catch (e: UnknownPropertyException) {
        throw SqlExParserException("配置文件错误, 未知的配置属性 ${e.path.toHumanReadableString()}, (位置 ${e.line}:${e.column})")
    }
}