...
impl StaticAnalyzer {
    fn as_execution_error(diagnostic: Diagnostic) -> sqlex_analyzer::AnalyzerError {
        diagnostic.to_execution_error()
    }

    fn as_analysis_error(diagnostic: Diagnostic) -> sqlex_analyzer::AnalyzerError {
        diagnostic.to_analysis_error()
    }
}
...
impl Analyzer for StaticAnalyzer {
    async fn execute(&mut self, sql: &str) -> Result<()> {
        let statements =
            parser::parse_statements(self.dialect, sql).map_err(Self::as_execution_error)?;
        let mutator = CatalogMutator::new(self.dialect);

        for statement in &statements {
            mutator
                .apply_statement(&mut self.catalog, statement)
                .map_err(Self::as_execution_error)?;
        }
