...
use clap::Parser;
use colored::*;
use sqlex_tests::{Result, TestRunner, runner::TestSuiteResult};
...
async fn main() -> Result<()> {
    ...
    if !fixtures_root.exists() {
        eprintln!(
            "{}: Fixtures root directory not found. Expected at 'crates/sqlex-tests/fixtures' or 'fixtures'",
            "Error".red().bold()
        );
        std::process::exit(1);
    }
    ...
    if !target_path.exists() {
        eprintln!(
            "{}: Path not found: {:?} (resolved from fixtures root)",
            "Error".red().bold(),
            target_path
        );
        std::process::exit(1);
                ...
                for i in 0..max_cols {
                    ...
                    match (db_col, sqlex_col) {
                        (Some(d), Some(s)) => {
                            ...
                            let status = if name_match && null_match {
                                "OK".green()
                            } else {
                                "DIFF".red()
                            };
                            ...
                            println!(
                                ...
                                format!(
                                    "{} ({})",
                                    d.type_name,
                                    if d.nullable { "NULL" } else { "NOT NULL" }
                                ),
                                format!(
                                    "{} ({})",
                                    s.type_name,
                                    if s.nullable { "NULL" } else { "NOT NULL" }
                                ),
                                status
                            );
                        },
