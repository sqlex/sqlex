...
class SqlExRepositoryToolWindow(private val project: Project) : Disposable {
    private val connect: MessageBusConnection = project.messageBus.connect()

    fun initAndGetContent(): JComponent {
        val mainContent = content
        connect.subscribe(
            SqlExRepositoryEventListener.REPOSITORY_SERVICE_TOPIC,
            object : SqlExRepositoryEventListener {
    ...
    private val content by lazy {
        SimpleToolWindowPanel(false, true).apply {
            ...
            setContent(OnePixelSplitter(false).apply {
                proportion = 0.2F
