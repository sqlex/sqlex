...
open class SqlExMethodParserDefinition : ParserDefinition {
    ...
    override fun createElement(node: ASTNode): PsiElement {
        ...
        return when (node.elementType) {
            ...
            RULE_METHOD -> MethodSubtree(node, RULE_METHOD_NAME)
            RULE_RETURN_TYPE -> ReturnTypeSubtree(node, node.elementType)
            RULE_METHOD_NAME -> MethodNameSubtree(node, TOKEN_ID)
            RULE_PAGED -> PagedSubtree(node, node.elementType)
            RULE_PARAM_LIST -> ParamListSubtree(node, node.elementType)
            RULE_PARAM -> ParamSubtree(node, RULE_PARAM_NAME)
            RULE_PARAM_NAME -> ParamNameSubtree(node, TOKEN_ID)
